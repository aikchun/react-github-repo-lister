...
import { fetchUserReposAndOrgs } from '../../actions/app-actions';
...
export const App = ({ fetchReposAndOrgs, isFetching, error, repos, orgs }) => {
  const classes = useStyles();
  const [searchQuery, setSearchQuery] = useState('');

  const handleInputChange = e => {
    setSearchQuery(e.target.value);
    fetchReposAndOrgs(e.target.value);
  };
...
App.propTypes = {
  ...
  fetchReposAndOrgs: PropTypes.func,
};

const mapDispatchToProps = dispatch => ({
  fetchReposAndOrgs: _.debounce(username => {
    dispatch(fetchUserReposAndOrgs(username));
  }, 500),
});
